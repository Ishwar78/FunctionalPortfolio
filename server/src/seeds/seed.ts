...
const seed = async () => {
  try {
    ...
    const adminPassword = await hashPassword(
      process.env.ADMIN_DEFAULT_PASSWORD || 'Ishwar@123'
    );
    const admin = new AdminUser({
      email: process.env.ADMIN_DEFAULT_EMAIL || 'ishwarsharma@example.com',
...
`,
    ...
    const contentSections = [
      ...
      },
      {
        key: 'social',
        content: {
          github: 'https://github.com/ishwarsharma',
          linkedin: 'https://linkedin.com/in/ishwarsharma',
          twitter: 'https://twitter.com/ishwarsharma',
          instagram: 'https://instagram.com/ishwarsharma',
        },
      },
      {
        key: 'banners',
